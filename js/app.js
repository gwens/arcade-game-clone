...
Gems.prototype.render = function(){
    ...
    for (var row = 0; row < numRows; row++) {
        for (var col = 0; col < numCols; col++) {
            var gemType = this.gemGrid[row][col];
            switch(gemType){
                case 0:
                break;
                case 1:
...
Player.prototype.update = function(dt){
    ...
    if (this.row === 0){
        this.level += 1;
        this.initialize();
        gems.initialize(); //ok referencing gems instance? ***********
    }
    //don't go off screen
    else if(this.row > 5){
        this.row = 5;
    ...
    //collect any gems
    var pickup = gems.gemGrid[this.row][this.col]; //ok referencing gems instance? ******
    switch(pickup){
        case 1:
        this.score += 1;
        break;
    ...
    //cell is now empty
    gems.gemGrid[this.row][this.col] = 0; //ok referencing gems instance? move this to Gems.update?
...
function checkCollisions(){
    if(player.invincible === false && gameState == 1){ //ok referencing player instance? ************
        for (i = 0; i < allEnemies.length; i++){ //ok referencing allEnemies instance? *********
...
document.addEventListener('keyup', function(e) {
    ...
    player.handleInput(allowedKeys[e.keyCode]); //this references player instance... *****
});
